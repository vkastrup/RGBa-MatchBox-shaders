//RGBa
// Dither -- Combined Dither Shader -- Matchbox Shader for Autodesk Flame
// Version 1.0
//
// Six dither algorithms in one shader:
//
//   Bayer 2x2  -- Smallest ordered matrix. Aggressive checkerboard at B&W,
//                 geometric grid pattern at higher levels.
//
//   Bayer 4x4  -- Classic ordered dither. Recognisable video-game screen look.
//
//   Bayer 8x8  -- Fine ordered matrix. Subtle at high levels, classic newspaper
//                 halftone quality at B&W.
//
//   White Noise -- Per-pixel hash. No spatial structure. Film-grain character.
//
//   IGN        -- Interleaved Gradient Noise. Smooth, directionless, less
//                 structured than Bayer. Best general-purpose noise dither.
//
//   Error Diffusion -- 1D approximation: x-sweep and y-sweep carry quantization
//                      error forward through a window of pixels; the current pixel
//                      gets half the accumulated x-error + half the y-error applied
//                      before its own quantization. Produces organic, ink-like edges.
//
// Color mode: applies the same dither threshold to each RGB channel independently.
// Luma mode:  converts to BT.709 luminance first, dithers that, outputs B&W.
//
// Temporal: shifts the threshold each frame by the golden ratio (0.6180339).
// Over time the dither pattern integrates to a smooth gradient.
...
void main() {
    ...
    // ------------------------------------------------------------------
    // Threshold-based modes (Bayer + noise)
    // ------------------------------------------------------------------
    float threshold = 0.5;
    ...
    // Temporal: shift threshold by golden ratio each frame.
    // Applies to threshold-based modes (0-4) only.
    if (temporal && algo < 5) {
    ...
    if (algo == 5) {
        // Error diffusion -- 1D sweep approximation.
        // Scan scan_size pixels left (x) and down (y), carry error forward,
        // apply half of each residual to the current pixel before quantizing.
